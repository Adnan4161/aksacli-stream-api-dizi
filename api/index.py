...
def _resolve_playerjs_embed(embed_url: str, page_url: str, session: requests.Session):
    ...
    dl_headers = {
        "User-Agent": USER_AGENT, "Referer": embed_url, "Origin": embed_origin,
...
def _resolve_stream_from_page(page_url: str, page_headers: dict):
    ...
    if direct: return direct, _origin(page_url) + "/"
    ...
    for raw_iframe in iframes[:10]:
        ...
        elif not if_url.startswith("http"): if_url = urljoin(page_url, if_url)

        if not _is_http_url(if_url): continue

        try:
            if_headers = {"User-Agent": USER_AGENT, "Referer": page_url, "Origin": _origin(page_url)}
            if_res = session.get(if_url, headers=if_headers, timeout=12)
            in_iframe = _extract_m3u8_from_text(if_res.text)
            if in_iframe: return in_iframe, _origin(if_url) + "/"
        except Exception:
            pass
...
def proxy_general():
    ...
    custom_headers = {
        "User-Agent": USER_AGENT, "Referer": "https://amp.tvkulesi.com/", "Origin": "https://amp.tvkulesi.com"
    }
    try:
...
def proxy_gold():
    ...
    try:
        ...
        return Response(res.content, mimetype='application/vnd.apple.mpegurl', headers={'Access-Control-Allow-Origin': '*'})
    except Exception: return redirect(url)
...
def stream_canli(kanal):
    ...
    if kanal in dogus:
        ref = dogus[kanal]
        local_headers = {"User-Agent": USER_AGENT, "Referer": ref, "Origin": _origin(ref)}
        link = fetch_dogus(dogus[kanal], local_headers)
        if link: return _redirect_no_cache(link)
...
def resolve_universal():
    ...
    if re.search(r'\.m3u8($|[?&])', target_url, re.IGNORECASE):
        return _redirect_no_cache(target_url)

    domain = _origin(target_url)
    custom_headers = {"User-Agent": USER_AGENT, "Referer": domain + "/", "Origin": domain}

    try:
        solved = _resolve_stream_from_page(target_url, custom_headers)
        if solved:
            stream_url, _ = solved
            return _redirect_no_cache(stream_url)
    except Exception:
        pass
...
def stream_dizi(dizi, bolum):
    ...
    if dizi in films:
        ...
        candidates.append(dizi_url)
        candidates.append(film_url)  # 404 hatasını önleyen kritik fallback
    ...
    fh_headers = {"User-Agent": USER_AGENT, "Referer": f"{base_domain}/", "Origin": base_domain}
    ...
    for candidate in ordered_candidates:
        try:
            ...
            if solved:
                stream_url, _ = solved
                return _redirect_no_cache(stream_url) # Sadece linki fırlatır
        except Exception:
            pass
...
@app.route('/')
def home():
    return "Aksaçlı Stream API V163.1 - Safe Redirects Active (Kota Dostu)"
